...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookUrl =
      process.env.N8N_NEGOCIO_CERRADO_WEBHOOK_URL ||
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "No se ha configurado la variable N8N_NEGOCIO_CERRADO_WEBHOOK_URL en .env.local",
          message:
    ...
    return NextResponse.json({
      success: true,
      message: "Formulario enviado exitosamente a n8n",
      data: responseData,
    });
  } catch (error: unknown) {
    const err = error as Error;
    console.error("Error procesando envío de formulario:", err);
    return NextResponse.json(
      {
        success: false,
